...
public final class ParserRegistryImpl implements ParserRegistry {
  ...
  @Override
  public <P extends Parseable, T extends ParseTask> void register(final Parser<P, T> parser) {
    parsers.put(parser.parseableClass(), parser);
  }

  @Override
  public <P extends Parseable, T extends ParseTask> Parser<P, T> getParser(final Class<Parser<P, T>> parserClass) {
    return CDI.current().select(parserClass).get();
  }

  @Override
  public <P extends Parseable, T extends ParseTask> P parse(final Class<Parser<P, T>> parserClass, final T task) {
    return getParser(parserClass).parse(task);
  }

  @Override
  public <P extends Parseable, T extends ParseTask> CompletableFuture<P> parseAsync(final Class<Parser<P, T>> parserClass, final T task) {
